...
- `TutorialsController` (attribute-routed: `/Tutorials` index, `/Tutorials/{slug}` detail) renders these JSONs via `ITutorialService`/`TutorialService` (reads `wwwroot/tutorials` fresh per request, no cache) and `Models/TutorialModels.cs`. `Views/Tutorials/Detail.cshtml` renders all 21 sections in Bootstrap tabs (Tutorial / MCQ Quiz / Exam / Flashcards / Interview / Glossary) plus interactive quiz. Nav menu key: `ActiveMenu = "GenAIDotNet"` ("GenAI .NET" item in both navbars in `_Layout.cshtml`).
- Tutorial page features (in `wwwroot/css/tutorial-theme.css` + `wwwroot/js/tutorial.js`, both loaded only by the Tutorials views): dark/light mode toggle (persisted, scoped to `.tut-scope`); step-based "animation" content blocks (pipeline diagrams with Play/Prev/Next — `nodes`+`steps` in the JSON, models `TutorialAnimNode`/`TutorialAnimStep`); double-click-to-add notes persisted in `localStorage` (key `tutorialNotes_<slug>`), restored on revisit, listed at the bottom (`#myNotes`), disabled inside the MCQ quiz tab; hover tooltips on technical terms driven by the JSON's `tooltips` map (first occurrence per section auto-marked by JS TreeWalker). All 27 prompt files carry a FEATURES footer requiring ≥2 animation blocks and verbatim tooltip terms.
